...
def _write_manifest(runs_dir, run_id, run_config, summary, duration_seconds=None):
...
def test_aggregate_includes_duration_seconds_column(tmp_path):
    runs_dir = tmp_path / "runs"
    _write_manifest(
        runs_dir, "run-1",
        {"pipeline_config_id": "p1", "segmentation_source_id": "baseline", "clustering_model": "cm1"},
    ...
    assert float(rows["run-1"]["duration_seconds"]) == 125.4


def test_aggregate_handles_manifest_missing_duration_seconds(tmp_path):
    """Older manifests (written before duration_seconds existed) have no
    top-level duration_seconds key at all -- aggregation must not KeyError
    on them, and should fall back to a blank cell."""
    runs_dir = tmp_path / "runs"
    runs_dir.mkdir(parents=True, exist_ok=True)
    old_manifest = {
        "run_id": "run-old",
        "created_at": "2026-09-07T12:00:00Z",
        "run_config": {"pipeline_config_id": "p1", "segmentation_source_id": "baseline", "clustering_model": "cm1"},
        "summary": {"der": 0.1},
        # no "duration_seconds" key at all
    }
    (runs_dir / "run-old.json").write_text(json.dumps(old_manifest))
    ...
    assert rows["run-old"]["duration_seconds"] == ""
