...
## Acceptance criteria
- [ ] With `identity`, the baseline run reproduces 17.05% **exactly**, not approximately -- **needs a real GPU/AMI run to verify, not yet executed**
- [ ] With `nearest_centroid`, the run completes and DER lands within roughly a point of baseline (a large swing means the plumbing is wrong) -- **needs a real GPU/AMI run to verify, not yet executed**
- [x] Both runs land in `comparison.csv` with correct `condition` values -- plumbing confirmed unit-wise (harness config now wires `refinement_strategy` -> `get_refinement_strategy` -> `pipeline.refinement`, and T1's `condition`/`refinement_strategy` fields flow into `comparison.csv` via `aggregate_runs.py`'s dynamic field union); the two real runs themselves haven't been executed yet
...
- The two remaining acceptance criteria (exact baseline reproduction with `identity`, nearest_centroid within ~1pt of baseline) still need a real GPU/AMI run -- not executed in this pass. Once run, both should appear correctly in `comparison.csv` given the plumbing above; `status` stays `in-progress` until those runs happen.
